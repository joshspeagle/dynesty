...
def test_sampling_history_completeness():
    ...
    try:
        ...
        with h5py.File(history_filename, 'r') as f:
            ...
            assert len(f['evaluation_u']) == evaluation_history_length, "sampling_u length mismatch"
            assert len(f['evaluation_v']) == evaluation_history_length, "sampling_v length mismatch"
            
            # Check data integrity: no NaN values
            assert not np.any(np.isnan(f['evaluation_logl'][:])), "sampling_logl contains NaN values"
            assert not np.any(np.isnan(f['evaluation_u'][:])), "sampling_u contains NaN values"
            assert not np.any(np.isnan(f['evaluation_v'][:])), "sampling_v contains NaN values"
...
def test_dynamic_sampling_history_completeness():
    ...
    try:
        ...
        sampler.run_nested(dlogz_init=0.1, print_progress=printing, maxiter=500)
